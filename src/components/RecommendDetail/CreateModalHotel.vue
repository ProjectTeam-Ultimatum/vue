      ...
      <div class="reply-modal">
        <div class="reply-modal-body">
          <div class="reply-info">
            <div>
                <img class="reply-image" :src="recommendHotelImgPath || 'default-image-url'" alt="Review Image">
            </div>
            <div class="reply-info-text">
                <h5>{{ recommendHotelTitle }} 방문하셨나요? :)</h5>
                <span>{{ recommendHotelIntroduction }}</span>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      rating: 0,
      recommendHotelImgPath: '',
      recommendHotelTitle: '',
      recommendHotelIntroduction: '',
      reply: {
        recommendHotelId: null,
        recommendReplyStar: '',
  ...
  computed: {
    feedbackOptions() {
      ...
      const options = {
        ...
        place: ['💙경치가 멋져요', '🚗접근성이 좋아요', '😊편의시설 잘 갖춰짐', '💛편안하게 즐겨요', '📸사진 찍기 좋아요']
      };
  ...
  methods: {
    async fetchHotelDetails() {
      if (!this.recommendHotelId) {
          console.error("recommendHotelId is not provided!");
          return;
      }
      console.log("전달 받은 HotelID:", this.recommendHotelId);  // 현재 ID 로깅
      try {
          const response = await this.$axios.get(`/api/recommend/listhotel/${this.recommendHotelId}`);
          if (response.data) {
          this.recommendHotelImgPath = response.data.recommendHotelImgPath;
          this.recommendHotelTitle = response.data.recommendHotelTitle;
          this.recommendHotelIntroduction = response.data.recommendHotelIntroduction;
          }
      } catch (error) {
    ...
    async createModal(){
      ...
      formData.append("recommendHotelId", this.reply.recommendHotelId);
      ...
      console.log("POST요청 푸드ID:", this.reply.recommendHotelId);
  ...
  try {
    const response = await this.$axios.post(`/api/recommendreply/hotel/save`, formData, {
      headers: {
    ...
    submitFeedback() {
      ...
      this.$router.push({ name: 'detailhotel', params: { recommendHotelId: this.recommendHotelId } }).catch(err => {
      console.error(err);
    });
    }, //submitFeedback
  },
    watch: {
        recommendHotelId(newVal) {
            if (newVal) {
                console.log("recommendHotelId has changed to:", newVal);  // ID 변경 감지 로그
  ...
  mounted() {
    // props 값을 data에 할당
    // props에서 받은 recommendHotelId를 data reply: { recommendHotelId: null}에 넣기
    this.reply.recommendHotelId = this.recommendHotelId;
    if (this.recommendHotelId) {
      console.log("Mounted with recommendHotelId:", this.recommendHotelId);  // 마운트 시 ID 로깅
      this.fetchHotelDetails();
    }
  },
